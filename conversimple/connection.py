...
- WebSocket connection with customer authentication
...
class WebSocketConnection:
    ...
    Implements the WebSocket protocol with customer authentication
    ...
    async def send_message(self, event: str, payload: Dict) -> None:
        """Send message to platform via WebSocket channel."""
    ...
    async def _join_channel(self) -> None:
        ...
        try:
            ...
            if isinstance(response_data, list):
                # WebSocket sends responses as arrays: [join_ref, ref, topic, event, payload]
                ...
                # Handle object format if WebSocket ever changes
    ...
    async def _handle_message(self, raw_message: str) -> None:
        ...
        try:
            ...
            # WebSocket message format: [join_ref, ref, topic, event, payload]
            if isinstance(message_data, list) and len(message_data) >= 4:
                ...
                if event == "phx_reply":
                    await self._handle_websocket_reply(payload)
                elif event == "phx_error":
                    logger.error(f"WebSocket error: {payload}")
                else:
    ...
    async def _handle_websocket_reply(self, payload: Dict) -> None:
        """Handle WebSocket reply messages."""
        status = payload.get("status")
        
        if status == "ok":
            logger.debug("WebSocket operation successful")
        elif status == "error":
            reason = payload.get("response", {}).get("reason", "unknown")
            logger.error(f"WebSocket operation failed: {reason}")
    ...
    def _next_message_ref(self) -> str:
        """Generate next message reference for WebSocket protocol."""
